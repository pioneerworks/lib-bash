...
Welcome to LibBash — an ever growing collection of scripts and mini-bash frameworks for doing all sorts of things quickly and efficiently.
...
> NOTE, in the following examples we assume you installed the library into your project's folder as `.lib-bash` (a "hidden" folder starting with a dot).
...
```bash
lib::url::shorten https://raw.githubusercontent.com/pioneerworks/lib-bash/master/bin/install
# http://bit.ly/2IIPNE1
```
...
#!/usr/bin/env bash
#
# Bootstrap LibBash into `bin/lib-bash` folder
curl -fsSL https://git.io/fxZSi | /usr/bin/env bash
source bin/lib-bash/Loader.bash
```


The installer above will do the following:

 * Checkout `lib-bash` repo into a folder above the current project — i.e. `../lib-bash`
 * If your project already has a `bin` folder — it's used, otherwise it's created.
 * The script will then create a symlink `./bin/lib-bash` pointing to `../lib-bash/lib`.
 * At this point, you should be able to source the library with `source bin/lib-bash/Loader.bash` and have all of the tools available to you.
...
lib::db::psql::args::homebase               lib::run::print-variables
...
Please see [existing tests](https://github.com/pioneerworks/lib-bash/tree/master/test) for the examples.
